...
(function(Opal) {
  Opal.dynamic_require_severity = "error";
  var self = Opal.top, $scope = Opal, nil = Opal.nil, $breaker = Opal.breaker, $slice = Opal.slice, $klass = Opal.klass, $hash2 = Opal.hash2, $range = Opal.range, state = nil, game = nil;

  Opal.add_stubs(['$require', '$new', '$add_text', '$set!', '$position', '$enabled_advanced_timing!', '$delta_time', '$game_size', '$each', '$velocity', '$sprite', '$[]', '$width', '$height', '$y=', '$+', '$y', '$*', '$x=', '$x', '$>', '$-', '$-@', '$<', '$key_down?', '$times', '$velocity=', '$rand', '$sprite=', '$add_sprite', '$<<', '$bring_to_top', '$text=', '$average_fps', '$size']);
  self.$require("gremlin");
  Opal.cdecl($scope, 'GRAVITY', 200);
  Opal.cdecl($scope, 'Bunny', $scope.get('Struct').$new("sprite", "velocity"));
  (function($base, $super) {
    function $MyState(){};
    var self = $MyState = $klass($base, $super, 'MyState', $MyState);
    ...
    def.$create = TMP_1 = function() {var $zuper = $slice.call(arguments, 0);
      var self = this, $iter = TMP_1.$$p, $yield = $iter || nil;

      TMP_1.$$p = null;
      Opal.find_super_dispatcher(self, 'create', TMP_1, $iter).apply(self, $zuper);
      self.bunnies = [];
      self.fps = self.$add_text("", $hash2(["fill"], {"fill": "white"}));
      self.fps.$position()['$set!'](20, 20);
      return self['$enabled_advanced_timing!']();
    };

    return (def.$update = function(args) {
      var $a, $b, TMP_2, $c, TMP_3, $d, self = this, dt = nil, gs = nil;

      args = $slice.call(arguments, 0);
      dt = self.$delta_time();
      gs = self.$game_size();
      ($a = ($b = self.bunnies).$each, $a.$$p = (TMP_2 = function(t){var self = TMP_2.$$s || this, $a, $b, vel = nil, pos = nil, size = nil, max_y = nil, max_x = nil;
if (t == null) t = nil;
      vel = t.$velocity();
        pos = t.$sprite().$position();
        size = (($scope.get('Gremlin')).$$scope.get('Point'))['$[]'](t.$sprite().$width(), t.$sprite().$height());
        ($a = vel, $a['$y=']($a.$y()['$+'](dt['$*']($scope.get('GRAVITY')))));
        ($a = pos, $a['$x=']($a.$x()['$+'](dt['$*'](vel.$x()))));
        ($a = pos, $a['$y=']($a.$y()['$+'](dt['$*'](vel.$y()))));
        max_y = pos.$y()['$+'](size.$y());
        if (max_y['$>'](gs.$y())) {
      ...
      if ((($a = self['$key_down?']((((($scope.get('Gremlin')).$$scope.get('Keyboard'))).$$scope.get('KEY_SPACEBAR')))) !== nil && (!$a.$$is_boolean || $a == true))) {
        ($a = ($c = (5)).$times, $a.$$p = (TMP_3 = function(){var self = TMP_3.$$s || this, $a, $b;
          if (self.bunnies == null) self.bunnies = nil;

        t = $scope.get('Bunny').$new();
          (($a = [(($scope.get('Gremlin')).$$scope.get('Point'))['$[]'](self.$rand($range(-200, 200, false)), 0)]), $b = t, $b['$velocity='].apply($b, $a), $a[$a.length-1]);
          ...
          return self.bunnies['$<<'](t);}, TMP_3.$$s = self, TMP_3), $a).call($c);
  ...
  state = $scope.get('MyState').$new();
  return game = (($scope.get('Gremlin')).$$scope.get('Game')).$new($hash2(["size", "state"], {"size": [800, 300], "state": state}));
})(Opal);
