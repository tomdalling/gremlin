...
if (cell_pos == null) cell_pos = nil;if (cell == null) cell = nil;
      num_frames = $scope.get('ANIMATIONS_FRAMES')['$[]'](cell.$type());
        ...
        return cell.$sprite().smoothed = false;}, TMP_27.$$s = self, TMP_27), $a).call($c);
...
if (e == null) e = nil;
      (($a = [self.$game().$add_sprite(e.$animation().$image_key()['$+'](e.$animation().$current_frame().$to_s()))]), $b = e, $b['$sprite='].apply($b, $a), $a[$a.length-1]);
        ...
        return e.$sprite().smoothed = false;}, TMP_28.$$s = self, TMP_28), $a).call($d);
...
if (m == null) m = nil;
  ...
  (function($base, $super) {
    ...
    def.$startup = function() {
      ...
      self.background = self.$game().$add_sprite("intro_background");
      return self.$game().stage.smoothed = false;;
    };
  ...
  state = $scope.get('GemmyGame').$new(starting_scene, $scope.get('ASSETS'));
  game = (($scope.get('Gremlin')).$$scope.get('Game')).$new($hash2(["size", "state"], {"size": [(13)['$*']($scope.get('GRID_SIZE')), (10)['$*']($scope.get('GRID_SIZE'))], "state": state}));
